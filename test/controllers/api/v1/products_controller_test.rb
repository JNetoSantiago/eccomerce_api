  ...
  module V1
    ...
    class ProductsControllerTest < ActionDispatch::IntegrationTest
      ...
      test 'should list all published products' do
        ...
        assert_equal json_response[:data].count, 2
        assert_equal [products(:one).id.to_s, products(:two).id.to_s],
                     [json_response.dig(:data, 0, :id), json_response.dig(:data, 1, :id)]
      end
      ...
      test 'should list for price greater than or equal to 500' do
        ...
        json_response = JSON.parse(response.body, symbolize_names: true)
        assert_equal json_response[:data].count, 2
        assert_equal [products(:two).id.to_s, products(:three).id.to_s],
                     [json_response.dig(:data, 0, :id), json_response.dig(:data, 1, :id)]
      end
      ...
      test 'should list for user' do
        get "#{api_v1_products_url}?q[user_id_eq]=#{users(:one).id}", as: :json
        assert_response :success
      ...
      test 'should list for category' do
        get "#{api_v1_products_url}?q[category_id_eq]=#{categories(:one).id}", as: :json
        assert_response :success
      ...
      test 'should show product' do
        ...
        assert_equal @product.category.description, json_response.dig(:included, 1, :attributes, :description)
      end

      test 'should create product' do
        assert_difference 'Product.count' do
          post api_v1_products_url,
               params: { product: { title: 'TV Lg 4k', price: 2286.63, published: true,
